...
export const dummyTransactions = Array.from({ length: 100 }, () => {
  ...
  const category = categories.length > 0 ? categories[Math.floor(Math.random() * categories.length)] : 'Other';
  
  return {
    id: uuidv4(),
    type: isExpense ? 'expense' : 'income',
    amount,
    description: `${isExpense ? 'Payment for' : 'Income from'} ${category ? category.toLowerCase() : 'other'}`,
